...
def get_items():
    ...
    for element in items:
        item_details = (f"{element['name'].center(20)}\t{str('{:03d}'.format(element['quantity']).center(10))}\t\t{element['unit'].center(7)}\t\t{str('{:.2f}'.format(element['unit_price']).center(15))}")
        print(item_details)

def add_item():
    print("Adding new item...")
    while True:
        i = str(input("Enter product's name: "))
        element = {}              
        if i.lower() == "quit":
            print("You have quit.") 
            break                          
        ...
        print()
        print("You have succesfully added a new product to the warehouse stock.\nHere's current status:")        
        get_items()
        break
